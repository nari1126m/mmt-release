...
Value evalExpr(const json &expr) {
	...
	if (!expr.is_object()) {
		cerr << "❌ expr ไม่ใช่ json object แต่เป็น: " << expr << endl;
		exit(1);
	}
	...
	// pimary
		...
		if (op == "NOT") {
			...
			cerr << "ไม่สามารถหา นิเสธของ " << valueToString(operand) << endl;
			exit(1);
			...
			cerr << "ไม่สามารถ สลับบิต ของ " << valueToString(operand) << endl;
			exit(1);
		} else if (op == "INCREMENT") {
			if (holds_alternative<int>(operand->data)) {
				return make_shared<ValueHolder>(get<int>(operand->data)++);
			}
			cerr << "ไม่สามารถ เพิ่มค่า ของ " << valueToString(operand) << endl;
			exit(1);
		} else if (op == "DECREMENT") {
			if (holds_alternative<int>(operand->data)) {
				return make_shared<ValueHolder>(get<int>(operand->data)--);
			}
			cerr << "ไม่สามารถ ลดค่า ของ " << valueToString(operand) << endl;
			exit(1);
		}
					...
					cerr << "ค่านี้ " << valueToString(operand) <<"ไม่สามารถติดลบได้"<< endl;
					exit(1);
				}
	} else if (type == "ln") {
		...
		if(holds_alternative<int>(indexVal->data) && get<int>(indexVal->data) >= 0){
			...
			if(d == static_cast<int>(d) && d>=0){
				...
				cerr<<"ผิดพลาด: ไม่สามารถเข้นถึงชุดข้อมูลด้วย ดัชนีที่เป็นทศนิยม ที่ บรรทัด "
					  << expr["line"] << ", คอลัมน์ " << expr["column"] <<endl;
				exit(1);
			}
		}else{
	    ...
	    if (!ns.empty()) {
	        ...
	        if (functions.find(funcname) == functions.end()) {
	            cerr << "โปรแกรม \"" << funcname << "\" ไม่พบในเนมสเปซ \"" << ns
	                 << "\" ที่บรรทัด " << expr["line"] << " คอลัมน์ " << expr["column"] << endl;
	            exit(1);
	        }
		...
		std::cerr << "เกิดข้อพิดพลาด: ขนาด() ไม่รองรับข้อมูลประเภทนี้ ที่บรรทัด : "
				  << expr["line"] << ", คอลัม์: " << expr["column"] << std::endl;
		exit(1);
	}
	cerr << "ไม่มี expression นี้ ที่ บรรทัด " << expr["line"]
		 << ", คอลัม์: " << expr["column"] << std::endl;
	exit(1);
}
...
Value evalStatement(const json &stmt) {
	if (!stmt.is_object()) {
		cerr << "❌ stmt ไม่ใช่ json object แต่เป็น: " << stmt << endl;
		exit(1);
	}
	  ...
	  return nullptr;
		...
		if(holds_alternative<ValueHolder::ArraY>(arrayVal->data)){
			...
			cerr << "ไม่สามารถเพิ่มสมาชิกเข้า  ชุดข้อมูลได้"
				 ...
				 << "ที่บรรทัด " << stmt["line"] << " คอลัมน์ " << stmt["column"] << endl;
			exit(1);
		}
		return nullptr;
	}
		...
		if (arr.empty()) {
			cerr << "ไม่สามารถ ดึงข้อมูลออก จาก ชุดข้อมูล ที่ว่าง "
				 << " ได้ที่บรรทัด " << stmt["line"] << " คอลัมน์ " << stmt["column"] << endl;
			exit(1);
		}
		...
		if(holds_alternative<ValueHolder::ArraY>(arrayVal->data)){
			...
			if (index < 0 || index > static_cast<int>(array.size())) {
				cerr << "ดัชนี อยู่นอกขอบเขตของ ชุดข้อมูล ที่บรรทัด " << stmt["line"] << " คอลัมน์ " << stmt["column"] << endl;
				exit(1);
			}
			...
			if (index < 0 || index > static_cast<int>(array.size())) {
				cerr << "ดัชนีอยู่นอกขอบเขตของข้อความ ที่บรรทัด "
				     << stmt["line"] << " คอลัมน์ " << stmt["column"] << endl;
				exit(1);
			}
	    ...
	    if (!fs::exists(filePath)) {
	        cerr << "ไม่พบไฟล์ '" << filePath << "' ที่บรรทัด " << stmt["line"] << " คอลัมน์ " << stmt["column"] << endl;
	        exit(1);
	    }

	    ifstream inFile(filePath);
	    if (!inFile.is_open()) {
	        cerr << "ไม่สามารถเปิดไฟล์ '" << filePath << "' ได้ ที่บรรทัด " << stmt["line"] << " คอลัมน์ " << stmt["column"] << endl;
	        exit(1);
	    }
	    ...
	    if (content.empty()) {
	        cerr << "ไฟล์ '" << filePath << "' ว่างเปล่า! ที่บรรทัด " << stmt["line"] << " คอลัมน์ " << stmt["column"] << endl;
	        exit(1);
	    }
	    ...
	    try {
	        ...
	        cerr << "ไม่สามารถ parse ไฟล์ '" << filePath << "' เป็น JSON ได้: " << e.what() << " ที่บรรทัด " << stmt["line"] << " คอลัมน์ " << stmt["column"] << endl;
	        exit(1);
	    }
...
public:
	...
	ASTNodePtr parseWhileloop() {
		...
		if (!match("WHILE")) {
			syntaxError(peek(), "ขาด คำสั่ง ทำซ้ำเมื่อ");
		}

		if (!match("OPEN_PAREN")) {
			syntaxError(peek(), "ขาด ( ที่ ทำซ้ำเมื่อ");
		}
	...
	ASTNodePtr parseDoWhile() {
		Token t = peek();
		if (!match("DO")) {
			syntaxError(peek(), "ไม่พบ ทำ ใน ทำ..ทำซ้ำเมื่อ");
		}

		if (!match("OPEN_BRACKETS")) {
			syntaxError(peek(), "ไม่พบ { ใน ทำ..ทำซ้ำเมื่อ");
		}
		vector<ASTNodePtr> body;
		while (peek().type != "CLOSE_BRACKETS" &&
			   peek().type !="EOF")
		...
		if (!match("WHILE")) {
			syntaxError(peek(), "ไม่พบ ทำซ้ำเมื่อ ใน ทำ..ทำซ้ำเมื่อ");
		}
		if (!match("OPEN_PAREN")) {
			syntaxError(peek(), "ไม่พบ ( ใน ทำ..ทำซ้ำเมื่อ");
		}
...
Value evalFunctionFromNode(const json &funcNode, const vector<Value> &args) {
	...
	if (funcNode["type"] != "functionDeclaretion") {
		cerr << "ไม่ใช่ฟังก์ชันที่สามารถเรียกได้" << endl;
		std::exit(1);
	}
	...
	if (params.size() != args.size()) {
		cerr << "จำนวน arguments ไม่ตรงกับ parameter" << endl;
		std::exit(1);
	}
...
int main(int argc, char *argv[]) {
    ...
    if (filepath.extension() != ".thl") {
        cerr << "Invalid file extension. Must be .thl" << endl;
        exit(1);
    }
    ...
    if (!fileTarget.empty()) {
        ...
        if (filepathTarget.extension() != ".json") {
            cerr << "ไฟล์เป้าหมายต้องเป็น .json เท่านั้น" << endl;
            exit(1);
        }
    }
    ...
    try {
        ...
        if (fileTarget.empty()) {
            ...
            if (ast_file.is_open()) {
                ...
                cerr << "Failed to write AST to: " << filepathTarget << endl;
                exit(1);
            }
        }
